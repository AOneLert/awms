...
myapp.AddEditProductReceivePurchaseOrderDetail.created = function (screen) {
    ...
    var recNo = screen.ProductReceivePurchaseOrderDetail.ReceiveNumber;

    if (recNo == undefined || recNo == null) {
        //screen.ProductReceivePurchaseOrderDetail.ReceiveNumber = "00000";
        myapp.activeDataWorkspace.ApplicationData.QueryMaxReceivePONumber().execute().then(function (results) {
            var docNo = results;
...
myapp.AddEditProductReceivePurchaseOrderDetail.RequireDate_postRender = function (element, contentItem) {
    // Write code here.
